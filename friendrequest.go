...
type OutgoingFriendRequest struct {
	...
	// ExpectedIdentity, when non-nil, is the recipient's expected hybrid
	// identity (Ed25519 + ML-DSA-65). Once the wire format moves to
	// introductionV2 (US1+US2 joint commit), ExpectedIdentity takes
	// precedence over ExpectedKey. Until then ExpectedKey is authoritative
	// and ExpectedIdentity is recorded for the upgrade.
	//
	// Field is not yet wired into client_json.go (easyjson). Persisted
	// requests round-trip without the PQ half until easyjson is regenerated;
	// the omission is intentional during the foundational migration.
	ExpectedIdentity *hybrid.HybridIdentityPublic `json:"-"`
...
type sentFriendRequest struct {
	...
	// MLKEMPublicKey and MLKEMPrivateKey are the per-friend-request
	// ephemeral ML-KEM-768 keypair, generated by the initiator alongside
	// the classical X25519 ephemeral. The pair is used by the responder
	// (via Encapsulate against MLKEMPublicKey) and the initiator (via
	// Decapsulate with MLKEMPrivateKey) to derive the PQ half of the
	// hybrid keywheel seed.
	//
	// These fields are dormant in v1 wire mode (genIntro/decodeAddFriendMessage
	// still ship the v1 introduction). They are populated and consumed once
	// the wire format moves to introductionV2 in the US1+US2 joint commit.
	//
	// Not yet wired into client_json.go (easyjson); persisted records
	// round-trip without these fields until easyjson is regenerated.
	MLKEMPublicKey  *pqkem.PublicKey  `json:"-"`
	MLKEMPrivateKey *pqkem.PrivateKey `json:"-"`
...
type IncomingFriendRequest struct {
	...
	// LongTermKeyPQ is the recipient's ML-DSA-65 long-term identity half
	// (the post-quantum sibling of LongTermKey). Populated once the wire
	// format moves to introductionV2 in the US1+US2 joint commit; nil
	// during the foundational v1 wire path.
	LongTermKeyPQ []byte `json:"-"`

	// MLKEMCiphertext, when non-nil, carries the ML-KEM-768 ciphertext
	// the recipient produced by encapsulating to the initiator's ML-KEM
	// public key (read out of the v2 introduction). The initiator
	// decapsulates against its sentFriendRequest.MLKEMPrivateKey to
	// recover the PQ half of the hybrid keywheel seed.
	//
	// Dormant in v1 wire mode. Not yet wired into easyjson.
	MLKEMCiphertext []byte `json:"-"`
